import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:get/get.dart';
import 'package:sslcquiz/controller/lesson/lesson_controller.dart';
import 'package:sslcquiz/controller/subject/subject_controller.dart';
import 'package:sslcquiz/screens/subject/subject_screen.dart';
import 'package:sslcquiz/utils/text_data.dart';

import '../../controller/theme/theme_controller.dart';
import '../../data/app_data.dart';
import '../../model/lesson_model.dart';
import '../../routes/app_pages.dart';
import '../../utils/colors.dart';
import '../../utils/constant.dart';
import '../../widgets/common_app_bar.dart';
import '../../widgets/common_text.dart';
import '../../widgets/constant_widget.dart';
import '../../widgets/spacing_widget.dart';

class LessonScreen extends StatefulWidget {
  const LessonScreen({super.key});

  @override
  State<LessonScreen> createState() => _LessonScreenState();
}

class _LessonScreenState extends State<LessonScreen> {
  // Use Get.find() instead of Get.put() since binding will create the controller
  late LessonController lessonController;

  @override
  void initState() {
    super.initState();
    // Get the controller instance created by binding
    lessonController = Get.find<LessonController>();
  }

  @override
  Widget build(BuildContext context) {
    setStatusBar();
    return WillPopScope(
      onWillPop: () async {
        Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => SubjectScreen(),));
        // Get.back();
        // Get.replace(AppRoutes.subjectScreen);
        return true;
      },
      child: Scaffold(
        body: Container(
          width: double.infinity,
          height: double.infinity,

          decoration: BoxDecoration(
            gradient: LinearGradient(
              begin: Alignment.topLeft,
              end: Alignment.bottomRight,
              colors: [AppColors.practiceTestOrange, Color(0xFFB86FD3)],
            ),
          ),

          child: Column(
            children: [
              CommonAppBar(
                backgroundColor: Theme.of(context).primaryColor,
                title: TextData.homeTitle,
                textColor: AppColors.backGroundColor,
                back: () {
                  Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => SubjectScreen(),));
                  // Get.replace(AppRoutes.subjectScreen);
                },
              ).paddingSymmetric(horizontal: 15.h),

              Expanded(
                child: Container(
                  width: double.infinity,
                  height: double.infinity,
                  margin: EdgeInsets.only(top: 15.h),
                  padding: EdgeInsets.only(top: 2.h, bottom: 15.h),

                  decoration: BoxDecoration(
                    color: Theme.of(context).scaffoldBackgroundColor,
                    borderRadius: BorderRadius.only(
                      topLeft: Radius.circular(25.h),
                      topRight: Radius.circular(25.h),
                    ),
                  ),
                  child: FutureBuilder(future: lessonController.getLessons(), builder: (context, snapshot) {


                    if(snapshot.connectionState == ConnectionState.waiting){
                      return Center(
                        child: SizedBox(
                          height: 30, // set your preferred size
                          width: 30,
                          child: CircularProgressIndicator(strokeWidth: 2),
                        ),
                      );
                    }
                    List<Lesson> lessons = snapshot.data?? [];

                    if(lessons.isEmpty){
                      return Center(
                        child: CommonText(
                          text: "No Data",
                          fontSize: AppDimensions.fontMedium,
                          color: AppColors.blackColor,
                          fontWeight: AppFontWeights.medium,
                        ),
                      );
                    }

                    return ListView.builder(
                      padding: EdgeInsets.symmetric(horizontal: 15.h),
                      itemCount: lessons.length,
                      itemBuilder: (context, index) {
                        Lesson e = lessons[index];


                        return _buildMenuItem(e.image, index, context, () async {
                          await AppData().setLesson(e.table);
                          Get.toNamed(
                            AppRoutes.questionScreen,
                            arguments: {'fromFavourite': false},
                          );
                        });
                      },
                    );
                  },),
                ),
              ),
            ],
          ),
        ),
      ),
    );
  }

  // Widget _buildMenuItem(
  //   String button,
  //   BuildContext context,
  //   Function function,
  // ) {
  //   return Center(
  //     child: GestureDetector(
  //       onTap: () {
  //         function();
  //       },
  //
  //       child: getImage(
  //         button,
  //         // height: 120.h,
  //         width: 200.w,
  //       ),
  //     ).marginOnly(bottom: 20.h),
  //   );
  // }

  Widget _buildMenuItem(
    String button,
    int index,
    BuildContext context,
    Function function,
  ) {
    return FutureBuilder<String>(
      future: AppData().getTitle(),
      builder: (context, asyncSnapshot) {

bool tamilTitle=false;
bool isEnglishSubject=false;

        if(asyncSnapshot.data!=null){
          tamilTitle =  asyncSnapshot.data == TextData.tamil;
          // Check if current subject is English
          isEnglishSubject = asyncSnapshot.data == TextData.english || asyncSnapshot.data == TextData.tEnglish;
        }

        return GestureDetector(
          onTap: () async {
            function();
          },

          child: Container(
            width: double.infinity,
            padding: AppDimensions.paddingAllMedium,
            decoration: BoxDecoration(
              color: Colors.white,

              boxShadow: [
                BoxShadow(
                  color: AppColors.slateGray.withOpacity(0.1), // Soft shadow
                  offset: Offset(0, 4), // Horizontal & vertical offset
                  blurRadius: 10, // How soft the shadow is
                  spreadRadius: 1, // How far the shadow spreads
                ),
              ],

              borderRadius: BorderRadius.all(Radius.circular(22.h)),
            ),

            child: Row(
              children: [
                Container(
                  height: 50.h,
                  width: 50.h,

                  decoration: BoxDecoration(
                    color: Color(0xFFDED7FF),

                    borderRadius: BorderRadius.all(Radius.circular(10.h)),
                  ),

                  alignment: Alignment.center,

                  child: CommonText(
                    text: '${index + 1}',
                    fontSize: AppDimensions.fontLarge,
                    color: AppColors.practiceTestOrange,
                    fontWeight: AppFontWeights.bold,
                  ),
                ),

                Spacing.width(20),

                Expanded(
                  child: Obx(() => CommonText(
                    // If English subject is selected, always show "Lesson" in English
                    // Otherwise follow language preference
                    text: isEnglishSubject
                        ? '${TextData.lesson} ${index + 1}'
                        : (lessonController.isTamil.value || tamilTitle ? '${TextData.tLesson} ${index + 1}':'${TextData.lesson} ${index + 1}'),
                    fontSize: AppDimensions.fontXMedium,
                    color: AppColors.blackColor,
                    fontWeight: AppFontWeights.bold,
                  ),)
                ),

                Icon(Icons.navigate_next, color: AppColors.practiceTestOrange),
              ],
            ),
          ),
          // child: getButtonImage(
          //   button,
          //   // height: 120.h,
          //   width: 150.w,
          // ),
        ).marginOnly(top: 15.h);
      }
    );
  }
}
